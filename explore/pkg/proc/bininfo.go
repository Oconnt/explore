...
import (
	...
	"explore/pkg/astutil"
	pdwarf "explore/pkg/dwarf"
	"explore/pkg/dwarf/frame"
	"explore/pkg/dwarf/godwarf"
	"explore/pkg/dwarf/line"
	"explore/pkg/dwarf/loclist"
	"explore/pkg/dwarf/op"
	"explore/pkg/dwarf/reader"
	"explore/pkg/goversion"
	"explore/pkg/internal/gosym"
	"explore/pkg/logflags"
	"explore/pkg/proc/debuginfod"
	"explore/pkg/proc/evalop"
...
func (bi *BinaryInfo) FindFileLocation(filename string, lineno int) ([]uint64, error) {
...
func (bi *BinaryInfo) Vars() []PackageVar {
	// verify if the type and size are consistent
	if reflect.TypeOf(packageVar{}).Size() != reflect.TypeOf(PackageVar{}).Size() {
		panic("inconsistent structure memory layout")
	}

	return *(*[]PackageVar)(unsafe.Pointer(&bi.packageVars))
}

func (bi *BinaryInfo) Constant() []ConstantValue {
	var res []ConstantValue
	for ref, constantsType := range bi.consts {
		values := constantsType.values
		for _, cv := range values {
			res = append(res, ConstantValue{
				Name:       cv.name,
...
func (cu *compileUnit) GetImage() *Image {
	return cu.image
}
