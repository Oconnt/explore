...
func NewConstant(name string, val constant.Value, mem MemoryReadWriter) *Variable {
	v := newConstant(val, mem)
	v.Name = name
	return v
}
...
func (v *Variable) maybeDereference() *Variable {
	...
	case *godwarf.PtrType:
		...
		r := v.newVariable("", ptrval, t.Type, v.mem)
		if err != nil {
			r.Unreadable = err
		}
...
func (v *Variable) loadPtr() {
	...
	if v.Unreadable == nil {
		ptrval, err := readUintRaw(v.mem, v.Addr, t.ByteSize)
		if err == nil {
			child = v.newVariable("", ptrval, t.Type, v.mem)
		} else {
	...
	if v.Unreadable != nil {
		...
		child = v.newVariable("", 0 /* addr */, t.Type, v.mem)
...
func (v *Variable) loadValueInternal(recurseLevel int, cfg LoadConfig) {
	...
	case reflect.String:
		...
		case v.Flags&VariableCPtr != 0:
			var done bool
			val, done, v.Unreadable = readCStringValue(v.mem, v.Base, cfg)
			if v.Unreadable == nil {
				v.Len = int64(len(val))
				if !done {
					v.Len++
...
func convertToEface(srcv, dstv *Variable) error {
	...
	if _, isiface := srcv.RealType.(*godwarf.InterfaceType); isiface {
		...
		return dstv.writeEmptyInterface(_type.Addr, data)
	}
...
func (v *Variable) writeEmptyInterface(typeAddr uint64, data *Variable) error {
	...
	fmt.Printf("dstType: %+v, dstData: %+v\n", dstType, dstData)
	if v.Unreadable != nil {
		return v.Unreadable
	}
...
func SetValue(dstv, srcv *Variable, srcExpr string) error {
